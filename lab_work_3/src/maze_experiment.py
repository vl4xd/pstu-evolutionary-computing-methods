...
# --- Настройка локального Graphviz ---
# Строим путь к папке bin вашего распакованного Graphviz
graphviz_bin_path = os.path.join(local_dir, 'Graphviz-14.1.5-win64', 'bin')
# Проверяем, что такая папка действительно существует
if os.path.exists(graphviz_bin_path):
    # Добавляем путь к bin в переменную окружения PATH
    os.environ["PATH"] = graphviz_bin_path + os.pathsep + os.environ.get("PATH", "")
    print(f"Graphviz bin path added to PATH: {graphviz_bin_path}")
else:
    print(f"Warning: Graphviz bin folder not found at {graphviz_bin_path}")
# -----------------------------------------------------
...
def run_experiment(config_file, maze_env, trial_out_dir, args=None, n_generations=100, silent=False):
    ...
    if not silent or solution_found:
        node_names =   {-1:'RF_R', -2:'RF_FR', -3:'RF_F', -4:'RF_FL', -5:'RF_L', -6: 'RF_B', 
                        ...
                        0:'ANG_VEL', 1:'VEL'}
        visualize.draw_net(config, best_genome, False, node_names=node_names, directory=trial_out_dir, fmt='svg')
        if args is None:
